...
class Evaluator:
    def __init__(
        ...
        if postprocessor_name == "actsub":
            if postprocessor.tune_decisive:
                self.hyperparam_search_decisive()
            else:
                postprocessor.decisive_search_done = True
    ...
    def hyperparam_search_decisive(self):
        ...
        for name in self.postprocessor.args_dict_decisive.keys():
            hyperparam_names.append(name)
            count += 1

        for name in hyperparam_names:
            hyperparam_list.append(self.postprocessor.args_dict_decisive[name])
        ...
        for i, hyperparam in enumerate(hyperparam_combination):
            self.postprocessor.set_hyperparam_decisive(hyperparam)

            id_pred, id_conf, id_gt = self.postprocessor.inference(
                self.net, self.dataloader_dict['id']['val'])
            
            id_pred_r, id_conf_r, id_gt_r = self.postprocessor.inference(
                self.net, self.dataloader_dict['csid']['imagenet_r'])
            
            ood_pred, ood_conf, ood_gt = self.postprocessor.inference(
                self.net, self.dataloader_dict['ood']['val'])

            ood_gt = -1 * np.ones_like(ood_gt)
            ood_gt_r = -1 * np.ones_like(ood_gt)
            ...
            pred_r = np.concatenate([id_pred_r, ood_pred])
            conf_r = np.concatenate([id_conf_r, ood_conf])
            label_r = np.concatenate([id_gt_r, ood_gt_r])
            ood_metrics_r = compute_all_metrics(conf_r, label_r, pred_r)


            auroc_fpr = ood_metrics[1] - ood_metrics[0]
            auroc_fpr_r = ood_metrics_r[1] - ood_metrics_r[0]
            auroc_fpr = (auroc_fpr + auroc_fpr_r)/2

            print('Hyperparam: {}, auroc-fpr: {}'.format(hyperparam, auroc_fpr))
            if auroc_fpr > max_auroc:
                final_index = i
                max_auroc = auroc_fpr

        self.postprocessor.set_hyperparam_decisive(hyperparam_combination[final_index])
        print('Final hyperparam: {}'.format(
            self.postprocessor.get_hyperparam_decisive()))
        self.postprocessor.decisive_search_done = True
